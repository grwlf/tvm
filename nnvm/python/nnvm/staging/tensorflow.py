...
from .common import stager, get_nnvm_op, AttrConverter as AttrConvert

__all__ = ['stage_tensorflow']
...
def _argx(func_name):
    ...
    def _impl(inputs, attr, params):
        ...
        return stager.stage(op_name, inputs,{'axis':axis_input_vlaue,'keepdims':False})
    return _impl
...
def _elemwise(name):
    def _impl(inputs, attr, *args):
        ...
        if op_name == 'broadcast_add' and inputs[0].attr('op_name') in conv_ops:
            ...
            inputs[1] = stager.stage('expand_dims', [inputs[1]], {'axis':axis, 'num_newaxis':2})
        return stager.stage(op_name, inputs,{'name':attr['_node_name']})
    return _impl
...
def _conv(opname):
    def _impl(inputs, attr, params):
        ...
        if attr['padding'] == 'VALID':
            ...
            if attr['data_format'] == 'NHWC':
                inputs[0] = stager.stage('pad', [inputs[0]],
                        {'pad_width':((0, 0),
                                    (pad_v[0], pad_v[1]),
                                    (pad_h[0], pad_h[1]),
                                    (0, 0))}
...
def _matmul():
    def _impl(inputs, attr, params):
        ...
        if attr['transpose_a']:
            inputs[0] = stager.stage('transpose',[inputs[0]], {'axis':axis(1, 0)})
        if not attr['transpose_b']:
            inputs[1] = stager.stage('transpose', [inputs[1]], {'axes':(1, 0)})
...
def _pack():
    def _impl(inputs, attr, params):
        axis = int(attr["axis"])
        inputs_reshaped = [stager.stage('expand_dims',[i], {'axis':axis, 'num_newaxis':1}) for i in inputs]
        return stager.stage('concatenate', inputs_reshaped, {'axis':axis})
...
def _bias_add():
    def _impl(inputs, attr, params):
        return stager.stage('broadcast_add', inputs[0:2])
    return _impl
...
def _relu6():
    def _impl(inputs, attr, params):
        return stager.stage('clip', [inputs[0]], {'a_min':0, 'a_max':6, 'name':attr['_node_name']})
    return _impl
...
def _square():
    def _impl(inputs, attr, params):
        return stager.stage('elemwise_mul', inputs[0:2])
    return _impl
...
def _stridedSlice():
    def _impl(inputs, attr, params):
        ...
        out = stager.stage('strided_slice', [inputs[0]], {'begin':begin, 'end':end, 'stride':stride})
        ...
        while out_index != len(out_shape):
            ...
            if (new_axis_mask & mask) and not ellipsis_mask & mask:
                ...
                out_index += 1
            index += 1
        return stager.stage('reshape', [out], {'shape':tuple(final_output)})
    return _impl
...
def _transpose():
    def _impl(inputs, attr, params):
        ...
        return stager.stage('transpose', [inputs[0]], {'axes':tuple(axes)})
    return _impl
...
def _rank():
    def _impl(inputs, attr, params):
        ...
        return stager.stage_var(name=name, shape=params[name].shape)
    return _impl
...
class GraphProto(object):
    ...
    def stage_tensorflow(self, graph):
        ...
        for node in graph.node:
            ...
            if node.op == "Placeholder":
                self._nodes[node.name] = stager.stage_var(name=node.name,
        ...
        if self._num_rnn_layer:
            out_rnn = stager.stage('concatenate', self._out_rnn, {'axis':0})
            out = [out, out_rnn]
    ...
    def _parse_param(self, key, value, name):
        ...
        if key == 'value':
            ...
            if np_array.dtype == np.dtype(object):
                ...
                self._nodes[name] = stager.stage_var(name=name)
                return
            ...
            self._nodes[name] = stager.stage_var(name=name, shape=self._params[name].shape)
        else:
    ...
    def _convert_operator(self, op_name, inputs, attrs,
        ...
        if op_name in identity_list:
            sym = stager.stage(op_name, inputs, attrs)
...
def stage_tensorflow(graph, outfile):
    ...
    stager.open(outfile)
    g = GraphProto()
    sym, params = g.stage_tensorflow(graph)
    stager.close()
    return sym, params
