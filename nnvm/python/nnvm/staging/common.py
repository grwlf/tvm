...
class Stager(object):
    ...
    def close(self):
        self.out.close()
    def name(self, symobj):
        return "".join([x if not x in ":\n/_" else "_" for x in symobj ])
    def embed_sym(self, s):
        return "sym_"+ self.name(str(s.handle.value))
    def embed_str(self, s):
        return "\"" + s + "\""
    def embed_const(self, c):
        if isinstance(c,str):
            return self.embed_str(c)
        else:
            return c

    def stage_out(self, out_sym, op_name, inputs, attrs):
        s = "%s = _sym.%s(%s)\n" % \
            ( self.embed_sym(out_sym)
            , op_name
            , ",".join(
                    [self.embed_sym(i) for i in inputs] +
                    ["%s=%s"%(aname,self.embed_const(aval)) for aname,aval in attrs.items()])
            )
        # print(s)
        self.out.write(s)

    def stage(self, op_name, inputs=[], attrs={}):
        res = get_nnvm_op(op_name)(*inputs, **attrs)
        self.stage_out(res, op_name, inputs, attrs)
        return res

    def stage_var(self, name, shape=None):
        if not shape is None:
            res = _sym.Variable(name=name, shape=shape)
            s = "%s = _sym.Variable(name=%s,shape=%s)\n" % \
                ( self.embed_sym(res)
                , self.embed_str(name)
                , shape
                )
        else:
            res = _sym.Variable(name=name)
            s = "%s = _sym.Variable(name=%s)\n" % \
                ( self.embed_sym(res)
                , self.embed_str(name)
                )
...
class AttrConverter(object):
    ...
    def __call__(self, inputs, attrs, *args):
        ...
        return stager.stage(op_name,inputs,new_attrs)
